...
class BasicDecodingBlock(torch.nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, groups=1, base_width=64, dilation=1,
...
class BottleneckDecoding(torch.nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, groups=1, base_width=64, dilation=1,
        ...
        self.interpolate = torch.nn.Upsample(scale_factor=stride, mode='bilinear', align_corners=False)
        self.conv2 = conv(width, width, kernel_size=3, stride=1, padding=dilation,
...
class ResNetDecoder(torch.nn.Module):
    ...
    def __init__(self, block, layers, channels, classes, zero_init_residual=False, strides=[1, 2, 2, 2],
                 ...
                 norm='BatchNorm2d', nonlin='ReLU', stem='CIFAR', upsample='B', convolution_type='Standard'):
        super().__init__()
        ...
        for idx, layer in reversed(list(enumerate(layers))):
            ...
            layer_list.append(self._make_layer(block, self.feature_width, layer, stride=strides[idx],
                                               dilate=replace_stride_with_dilation[idx], upsample=upsample))
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, upsample='B'):
        ...
        if stride != 1:
            ...
            if upsample == 'A':
                upsample_op = torch.nn.Sequential(
                    ...
                    conv_layer(planes * block.expansion, self.target_width, kernel_size=1, stride=1, bias=self.use_bias),
                )
            elif upsample == 'B':
                upsample_op = torch.nn.Sequential(
                    torch.nn.Upsample(scale_factor=stride, mode='bilinear', align_corners=False),
        ...
        for _ in range(1, blocks):
            layers.append(block(self.feature_width, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, nonlin=nonlin_layer, bias=self.use_bias))

        layers.append(block(self.target_width, planes, stride, upsample_op, self.groups,
...
def generate_decoder(original_model):
    ...
    model = ResNetDecoder(BasicDecodingBlock, layers, 3, 10, stem='CIFAR',
                          convolution_type='Standard',
                          nonlin='ReLU', norm='BatchNorm2d',
                          upsample='B', width_per_group=64,
