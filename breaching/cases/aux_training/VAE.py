...
class VQ_VAE(torch.nn.Module):
    ...
    def loss_function(self, x, recon_x, z_e, emb, data_mean=0.0, data_std=1.0):
        B = x.shape[0]
...
class VQ_CVAE(torch.nn.Module):
    ...
    def loss_function(self, x, recon_x, z_e, emb, argmin, data_mean=0.0, data_std=1.0):
        mse = F.mse_loss(recon_x, x) * data_std
...
def train_encoder_decoder(encoder, decoder, dataloader, setup, arch='VAE'):
    ...
    for epoch in range(epochs):
        epoch_loss, epoch_mse, epoch_test = 0, 0
        for idx, (data, label) in enumerate(dataloader):
            model.train()
            optimizer.zero_grad(set_to_none=True)
            data = data.to(**setup)

            reconstructed_data, vars = model(data, noise_level=1.0)
            loss = model.loss(data, reconstructed_data, *vars, data_mean=data_mean, data_std=data_std)
            loss.backward()
            with torch.inference_mode():
                model.eval()
                epoch_test += model(data, noise_level=0.0)
