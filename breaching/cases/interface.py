...
def construct_case(cfg_case, setup, dryrun=False):
    ...
    model = construct_model(cfg_case.model, cfg_case.data, pretrained=cfg_case.server_model_state == 'trained')
    loss = torch.nn.CrossEntropyLoss()

    if cfg_case.server == 'honest_but_curious':
        server = HonestServer(model, loss, cfg_case.server_model_state, cfg_case.num_queries, cfg_case.data)
    else:
        raise ValueError(f'Invalid server settings {cfg_case.server} given.')

    if cfg_case.num_local_updates == 1:
