...
class Birdsnap(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.crop_to_bbx:
            img = img.crop((self.meta[index]['bb_x1'], self.meta[index]['bb_y1'],
                            self.meta[index]['bb_x2'], self.meta[index]['bb_y2']))
        img = img.convert("RGB")
    ...
    def scrape_images(self, missing_ids, chunk_size=8196):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:  # Choose max_workers dynamically
            ...
            future_to_url = {executor.submit(_load_url_and_save_image, idx, 600): self.meta[idx]['url'] for idx in missing_ids}
